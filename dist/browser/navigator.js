...
exports.collectSavedPostsHtml = collectSavedPostsHtml;
 ...
 */
async function collectSavedPostsHtml(context, savedUrl, stopPostIds, maxIterations, scrollDelayMin, scrollDelayMax, onProgress) {
        ...
        for (let i = 0; i < maxIterations; i++) {
            ...
            if (stopIdsArray.length > 0) {
                const found = await page.evaluate((ids) => {
                    return ids.some((id) => !!document.querySelector(`[data-chameleon-result-urn*="${id}"]`));
                }, stopIdsArray);
                if (found)
                    break;
            }
            // Count current posts
            const currentCount = await page.evaluate(() => document.querySelectorAll("[data-chameleon-result-urn]").length);
